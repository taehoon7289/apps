...
import com.example.app_drawer.code.ButtonState
...
class SwipeController() : ItemTouchHelper.Callback() {
    ...
    private fun drawButtons(
        ...
        if (buttonsShowedState === ButtonState.LEFT_VISIBLE) {
            ...
            p.color =
                Color.parseColor("#eeeeee") //Paint 객체 컬러 지정, setColor 메서드는 int형 인자를 매개변수로 받기 때문에 string 형태의 색상을 Color 클래스의 parseColor 메서드를 이용해 int형으로 바꿔줌
            c.drawRoundRect(leftButton, corners, corners, p) //drawRoundRect : 원 그리기
            drawText("왼쪽 eeeeee", c, leftButton, p)
            ...
            p.color = Color.parseColor("#2b2b2b")
            c.drawRoundRect(rightButton, corners, corners, p)
            drawText("오른쪽 #2b2b2b", c, rightButton, p)
